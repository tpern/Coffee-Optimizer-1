...
// =====================================================
const FARMER_CODE = "URENA";

function validateFarmerCode(code) {
  return code && code.toUpperCase().trim() === FARMER_CODE;
}

function activateFarmerAccess() {
  localStorage.setItem(STORAGE_KEYS.accessLevel, "farmer");
  console.info("Farmer access activated");
}

function checkAccessLevel() {
  const stored = localStorage.getItem(STORAGE_KEYS.accessLevel);
  return stored || "free"; // Returns: "free", "pro", or "farmer"
}

// =====================================================
// PRIVACY & DATA COLLECTION
// =====================================================
function savePrivacyConsent(consented, dataSharingEnabled) {
...
function handleFarmerCodeSubmit(codeInput) {
  ...
  if (validateFarmerCode(code)) {
    activateFarmerAccess();
...
function savePrivacySettings(consented, dataSharingEnabled) {
  savePrivacyConsent(consented, dataSharingEnabled);
...
function checkFirstTimeConsent() {
  ...
  if (!consent) {
    // Show consent modal (if modal exists in HTML)
    ...
    if (modal) {
      ...
      savePrivacyConsent(consented, dataSharing);
    }
  }
}
...
document.getElementById("feedback-submit-btn").addEventListener("click", function () {
  ...
  // Gather additional brew data from form fields if available
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Ensure pressure profile feedback is updated if machine/profile already selected
  if (espressoMachineSelect && espressoMachineSelect.value) {
    updatePressureProfileFeedback();
  }
  
  console.info("Coffee Optimizer initialized");
});
